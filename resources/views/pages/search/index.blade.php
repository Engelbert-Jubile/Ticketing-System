...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-8">
        <a href="{{ url()->previous() }}" class="inline-flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors mb-3">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 search-grid">
        ...
        {{-- TASKS (✅ bisa diklik ke detail) --}}
        @foreach($tasks as $task)
        <a data-card-link="1" wire:navigate.hover href="{{ route('tasks.show', ['taskSlug' => $task->public_slug]) }}"
            class="flex flex-col bg-white dark:bg-gray-800 rounded-lg shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer">
            <div class="p-5 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center gap-3">
            ...
            <div class="px-5 py-3 bg-gray-50 dark:bg-gray-800/50 rounded-b-lg border-t border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <span class="status-badge {{ $bgColor }}">
                        {{ $task->status_label ?? \Illuminate\Support\Str::of($statusValue)->replace('_',' ')->title() }}
                    </span>
                    @endif

                    @if(!empty($task->requester))
                    <span class="font-medium">Oleh: {{ e($task->requester->display_name ?? $task->requester->name ?? '—') }}</span>
                    @endif
