...
<div class="min-h-screen flex items-center justify-center bg-red-500 p-4">
...
<form id="login-form" method="POST" action="{{ route('login.store', ['locale' => $currentLocale]) }}" class="space-y-4">
  ...
  <input id="password" ... />
<a href="https://wa.me/62895393358741?text={{ urlencode('Halo, saya ingin reset password akun Tickora. Email saya: ' . old('email')) }}"
   onclick="return confirm('Anda akan diarahkan ke WhatsApp untuk mengirim permintaan reset password. Lanjutkan?')"
   target="_blank" rel="noopener"
   class="mt-2 block text-right text-sm text-blue-600 hover:underline">
  Lupa Password?
</a>
...
<div>
    ...
    <style>
      :root{--lo-primary:#1d4ed8;--lo-text:#334155;--lo-text-strong:#0f172a;--lo-bg-surface:rgba(255,255,255,.75)}
      @media (prefers-color-scheme: 
dark){:root{--lo-primary:#60a5fa;--lo-text:#334155;--lo-text-strong:#0f172a;--lo-bg-surface:rgba(255,255,255,.95)}}
        	
      #loginOverlay{position:fixed;inset:0;background:#fff !important;backdrop-filter:none !important;-webkit-backdrop-filter:none 
!important;z-index:999999;opacity:0;visibility:hidden;transition:opacity .18s ease,visibility .18s ease;pointer-events:none}     
    ...
    <div id="loginOverlay" aria-hidden="true" style="background: rgba(255,255,255,.92) !important; backdrop-filter:none !important; 
